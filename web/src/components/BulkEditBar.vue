<template>
  <div class="flex items-center gap-3 flex-wrap">
    ...
    <div class="flex items-center gap-2 min-w-0">
      ...
      >
        ...
        <optgroup :label="t('bulk.systemFields')">
          <option v-for="f in systemFields" :key="f.key" :value="f.key">{{ f.label }}</option>
        </optgroup>
        <optgroup v-if="customFields.length > 0" :label="t('bulk.contentFields')">
    ...
    >
      ...
      {{ applying ? t('bulk.applying') : t('bulk.applyEntries', { count: selectedCount, itemLabel: t(selectedCount === 1 ? 'contentList.entry' : 'contentList.entries') }) }}
    </button>
  </div>
</template>
...
const systemFields = computed(() => [
  ...
  { key: 'published_at', label: t('contentEdit.publishedAt'), kind: 'datetime' },
  { key: 'scheduled_at', label: t('contentEdit.scheduledFor'), kind: 'datetime' },
])

// Field types that have a sensible inline bulk-edit control
const SUPPORTED_TYPES = new Set(['text', 'slug', 'textarea', 'number', 'range', 'boolean', 'date', 'datetime', 'select'])
...
const selectedField = computed(() => allFields.value.find(f => f.key === selectedKey.value) ?? null)

// ── Apply logic ───────────────────────────────────────────────────────────────

const canApply = computed(() => {
  if (!selectedField.value) return false
  if (fieldValue.value === null || fieldValue.value === '') return false
  if (Array.isArray(fieldValue.value) && fieldValue.value.length === 0) return false
  return true
})
