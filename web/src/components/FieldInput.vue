<template>
  <!-- readonly preview -->
  <div v-if="readonly" class="min-h-10 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-800">
    <div v-if="isEmptyValue" class="text-slate-400">—</div>

    <div v-else-if="config.type === 'boolean'" class="inline-flex items-center gap-2">
      <Icon :icon="modelValue ? 'mdi:check-circle' : 'mdi:close-circle'" :class="modelValue ? 'text-emerald-600' : 'text-slate-400'" class="h-4 w-4" />
      <span>{{ modelValue ? 'true' : 'false' }}</span>
    </div>

    <div v-else-if="config.type === 'color'" class="inline-flex items-center gap-2">
      <span class="h-5 w-5 rounded border border-slate-300 shadow-sm" :style="{ backgroundColor: colorHex }"></span>
      <span class="font-mono">{{ colorHex }}</span>
    </div>

    <div v-else-if="config.type === 'markdown'" class="prose-preview" v-html="readonlyMarkdownHtml"></div>

    <pre v-else-if="config.type === 'json'" class="whitespace-pre-wrap break-words font-mono text-xs">{{ jsonValue }}</pre>

    <div v-else-if="config.type === 'media'" class="space-y-2">
      <div class="flex flex-wrap gap-2">
        <a v-for="file in mediaValues" :key="file" :href="mediaUrl(file)" target="_blank" rel="noreferrer"
          ...
          <span class="flex h-8 w-8 shrink-0 items-center justify-center overflow-hidden rounded bg-slate-100">
            <img v-if="isImage(file)" :src="mediaUrl(file)" :alt="file" class="h-full w-full object-cover" />
    ...
    <div v-else-if="config.type === 'repeater'" class="space-y-2">
      <div v-for="(row, rowIndex) in repeaterRows" :key="row._rid" class="rounded-lg border border-slate-200 bg-white p-3">
        <div class="mb-2 text-xs font-semibold uppercase text-slate-400">Row {{ rowIndex + 1 }}</div>
        <div class="grid gap-3" :class="subFieldGridCols">
          <div v-for="sub in (config.subfields ?? [])" :key="sub.key">
            <div class="mb-1 text-xs font-medium text-slate-500">{{ sub.label || sub.key }}</div>
            <FieldInput :name="`${name}[${rowIndex}][${sub.key}]`" :config="sub" :model-value="row[sub.key] ?? null" readonly />
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-else-if="Array.isArray(modelValue)" class="flex flex-wrap gap-1.5">
      <span v-for="item in modelValue" :key="String(item)" class="rounded-md bg-white px-2 py-1 text-xs text-slate-700 ring-1 ring-slate-200">{{ item }}</span>
    </div>
  ...
  <!-- text / slug / number -->
  <input v-else-if="['text', 'slug', 'number'].includes(config.type)" :type="config.type === 'number' ? 'number' : 'text'"
    :name="name" :value="modelValue ?? ''" class="form-input w-full rounded-lg border-slate-300 text-sm"
    @input="$emit('update:modelValue', $event.target.value)" />

  <!-- range -->
  <div v-else-if="config.type === 'range'" class="space-y-2">
    <div class="flex items-center gap-3">
      <input type="range" :name="name" :min="rangeMin" :max="rangeMax" :step="rangeStep" :value="rangeValue"
        class="w-full accent-theme-600" @input="emitRangeValue($event.target.value)" />
      <output
  ...
  <!-- textarea -->
  <textarea v-else-if="config.type === 'textarea'" :name="name" :value="modelValue ?? ''" rows="4"
  ...
  <div v-else-if="config.type === 'markdown'" class="space-y-1.5">
    <div class="flex items-center justify-end gap-1">
      <button type="button" class="text-xs px-2.5 py-1 rounded-md border transition-colors"
        :class="markdownMode === 'visual' ? 'border-theme-400 bg-theme-50 text-theme-700 font-medium' : 'border-slate-200 text-slate-500 hover:border-slate-300'"
        @click="setMarkdownMode('visual')">Visual</button>
      <button type="button" class="text-xs px-2.5 py-1 rounded-md border transition-colors"
        :class="markdownMode === 'raw' ? 'border-theme-400 bg-theme-50 text-theme-700 font-medium' : 'border-slate-200 text-slate-500 hover:border-slate-300'"
        @click="setMarkdownMode('raw')">Raw</button>
    </div>

    <textarea v-if="markdownMode === 'raw'" :name="name" :value="modelValue ?? ''" rows="10"
      class="form-textarea w-full rounded-lg border-slate-300 text-sm font-mono"
      @input="$emit('update:modelValue', $event.target.value)" />
    ...
    <div v-else
      ...
      <div class="flex flex-wrap gap-0.5 p-1.5 border-b border-slate-200 bg-slate-50">
        <button type="button" class="tiptap-btn font-bold" title="Bold"
          @click="editor?.chain().focus().toggleBold().run()"
          :class="{ 'is-active': editor?.isActive('bold') }">B</button>
        <button type="button" class="tiptap-btn italic" title="Italic"
          @click="editor?.chain().focus().toggleItalic().run()"
          :class="{ 'is-active': editor?.isActive('italic') }">I</button>
        <button type="button" class="tiptap-btn" title="Strikethrough"
          @click="editor?.chain().focus().toggleStrike().run()"
          :class="{ 'is-active': editor?.isActive('strike') }"><s>S</s></button>
        <span class="w-px bg-slate-200 mx-0.5 self-stretch"></span>
        <button type="button" class="tiptap-btn" title="Heading 1"
          @click="editor?.chain().focus().toggleHeading({ level: 1 }).run()"
          :class="{ 'is-active': editor?.isActive('heading', { level: 1 }) }">H1</button>
        <button type="button" class="tiptap-btn" title="Heading 2"
          @click="editor?.chain().focus().toggleHeading({ level: 2 }).run()"
          :class="{ 'is-active': editor?.isActive('heading', { level: 2 }) }">H2</button>
        <button type="button" class="tiptap-btn" title="Heading 3"
          @click="editor?.chain().focus().toggleHeading({ level: 3 }).run()"
          :class="{ 'is-active': editor?.isActive('heading', { level: 3 }) }">H3</button>
        <span class="w-px bg-slate-200 mx-0.5 self-stretch"></span>
        <button type="button" class="tiptap-btn" title="Bullet list"
          @click="editor?.chain().focus().toggleBulletList().run()"
          :class="{ 'is-active': editor?.isActive('bulletList') }">• List</button>
        <button type="button" class="tiptap-btn" title="Ordered list"
          @click="editor?.chain().focus().toggleOrderedList().run()"
          :class="{ 'is-active': editor?.isActive('orderedList') }">1. List</button>
        <span class="w-px bg-slate-200 mx-0.5 self-stretch"></span>
        <button type="button" class="tiptap-btn" title="Blockquote"
          @click="editor?.chain().focus().toggleBlockquote().run()"
          :class="{ 'is-active': editor?.isActive('blockquote') }">"</button>
        <button type="button" class="tiptap-btn font-mono text-xs" title="Inline code"
          @click="editor?.chain().focus().toggleCode().run()"
          :class="{ 'is-active': editor?.isActive('code') }">&lt;/&gt;</button>
        <button type="button" class="tiptap-btn font-mono text-xs" title="Code block"
          @click="editor?.chain().focus().toggleCodeBlock().run()"
          :class="{ 'is-active': editor?.isActive('codeBlock') }">```</button>
        <span class="w-px bg-slate-200 mx-0.5 self-stretch"></span>
        <button type="button" class="tiptap-btn" title="Horizontal rule"
          @click="editor?.chain().focus().setHorizontalRule().run()">—</button>
        <button type="button" class="tiptap-btn" title="Undo" @click="editor?.chain().focus().undo().run()">↩</button>
        <button type="button" class="tiptap-btn" title="Redo" @click="editor?.chain().focus().redo().run()">↪</button>
      </div>
  ...
  <!-- boolean -->
  <ToggleSwitch v-else-if="config.type === 'boolean'" :model-value="!!modelValue"
    @update:model-value="$emit('update:modelValue', $event)" />

  <!-- date -->
  <input v-else-if="config.type === 'date'" type="date" :name="name" :value="modelValue ?? ''"
    class="form-input w-full rounded-lg border-slate-300 text-sm"
    @input="$emit('update:modelValue', $event.target.value)" />

  <!-- datetime -->
  <input v-else-if="config.type === 'datetime'" type="datetime-local" :name="name" :value="modelValue ?? ''"
    class="form-input w-full rounded-lg border-slate-300 text-sm"
    @input="$emit('update:modelValue', $event.target.value)" />

  <!-- select -->
  <SearchableSelect v-else-if="config.type === 'select' && config.multiple"
    :options="(config.options ?? []).map(o => ({ value: o, label: o }))"
    :model-value="normalizeChoiceValues(modelValue)" :multiple="true" placeholder="Select options…"
    @update:model-value="$emit('update:modelValue', $event)" />

  <select v-else-if="config.type === 'select'" :name="name" :value="normalizeChoiceValue(modelValue) ?? ''"
  ...
  <div v-else-if="config.type === 'relation'">
    <SearchableSelect :options="relationOptions" :model-value="relationValue" :multiple="!!config.multiple"
      :placeholder="config.target ? 'Select entry…' : 'Choose a target post type first'" :disabled="!config.target"
      :loading="relationLoading" @update:model-value="$emit('update:modelValue', $event)" @search="onRelationSearch" />
  </div>
  ...
  <div v-else-if="config.type === 'media'" class="space-y-2">
    <div class="flex items-start gap-3">
      <div v-if="config.multiple"
        ...
        :class="isMediaDropActive ? 'border-theme-400 bg-theme-50 ring-2 ring-theme-200' : ''"
        @dragenter.prevent="onMediaDragEnter" @dragover.prevent="onMediaDragOver" @dragleave.prevent="onMediaDragLeave"
        @drop.prevent="onMediaDrop">
        <div v-for="file in mediaPreviewTiles" :key="file"
          class="overflow-hidden rounded-md bg-slate-200 flex items-center justify-center">
          <img v-if="isImage(file)" :src="mediaUrl(file)" class="h-full w-full object-cover" :alt="file" />
          <Icon v-else v-bind="getFileIcon(file)" class="w-5 h-5" />
        </div>
        <div v-if="mediaPreviewOverflow > 0"
          class="rounded-md bg-slate-900/75 text-white text-xs font-semibold flex items-center justify-center">
          +{{ mediaPreviewOverflow }}
        </div>
        <div v-for="index in mediaPreviewEmptyTiles" :key="`media-empty-${index}`" class="rounded-md bg-slate-200/70">
        </div>
      </div>
      ...
      <div v-else
        ...
        <template v-if="mediaValues.length > 0 && isImage(mediaValues[0])">
          <img :src="mediaUrl(mediaValues[0])" class="w-full h-full object-cover" :alt="String(mediaValues[0])" />
        </template>
      ...
      <div class="min-w-0 flex-1 space-y-2">
        <input v-for="file in mediaValues" :key="file" type="hidden" :name="`${name}[]`" :value="file" />
        <input v-if="mediaValues.length === 0" type="hidden" :name="name" value="" />
        <p class="text-sm text-slate-700 truncate" :title="String(modelValue ?? '')">
          {{ mediaLabel }}
        </p>
        <div class="flex flex-wrap gap-2">
          <button type="button" class="btn-secondary"
            :class="isMediaDropActive ? 'ring-2 ring-theme-300 ring-offset-1' : ''" :disabled="mediaUploading"
            @click="pickerOpen = true" @dragenter.prevent="onMediaDragEnter" @dragover.prevent="onMediaDragOver"
            @dragleave.prevent="onMediaDragLeave" @drop.prevent="onMediaDrop">
            {{ config.multiple ? 'Choose media files' : 'Choose media' }}
          </button>
          <button v-if="mediaValues.length > 0" type="button" class="btn-secondary"
            @click="$emit('update:modelValue', [])">
            Clear
          </button>
        </div>
        <p v-if="mediaUploading" class="text-xs text-theme-700">Uploading dropped file{{ config.multiple ? 's' : ''
          }}...</p>
        <p v-else-if="isMediaDropActive" class="text-xs text-theme-700">Drop to upload and select</p>
        <p v-if="mediaUploadError" class="text-xs text-red-600">{{ mediaUploadError }}</p>
      </div>
    </div>

    <!-- Reorderable selected files list (multiple mode) -->
    <div v-if="config.multiple && mediaValues.length > 0" class="overflow-hidden rounded-lg border border-slate-200">
      <div v-for="(file, index) in mediaValues" :key="file"
        class="flex items-center gap-2 border-b border-slate-100 bg-white px-2 py-1.5 last:border-0 transition-colors"
        :class="reorderDropIdx === index && reorderDragIdx !== index ? 'bg-theme-50 ring-1 ring-inset ring-theme-300' : ''"
        draggable="true" @dragstart="onMediaItemDragStart(index, $event)" @dragover.prevent="reorderDropIdx = index"
        @dragend="onMediaItemDragEnd" @drop.prevent="onMediaItemDrop(index)">
        <Icon icon="mdi:drag-vertical" class="h-4 w-4 shrink-0 cursor-grab text-slate-400 active:cursor-grabbing" />
        <div class="flex h-8 w-8 shrink-0 items-center justify-center overflow-hidden rounded bg-slate-100">
          <img v-if="isImage(file)" :src="mediaUrl(file)" :alt="file" class="h-full w-full object-cover" />
          <Icon v-else v-bind="getFileIcon(file)" class="h-4 w-4" />
        </div>
        <span class="min-w-0 flex-1 truncate text-xs text-slate-700" :title="file">{{ file }}</span>
        <button type="button" class="shrink-0 text-slate-400 transition-colors hover:text-red-500"
    ...
    <MediaPickerModal v-if="pickerOpen" :selected="mediaValues" :multiple="!!config.multiple"
      @close="pickerOpen = false" @select="selectMedia" />
  </div>

  <!-- json -->
  <textarea v-else-if="config.type === 'json'" :name="name" :value="jsonValue" rows="5"
    class="form-textarea w-full rounded-lg border-slate-300 text-sm font-mono"
    @input="onJsonInput($event.target.value)" />

  <!-- repeater -->
  <div v-else-if="config.type === 'repeater'" class="space-y-2">
    <div v-for="(row, rowIndex) in repeaterRows" :key="row._rid"
      ...
      <div
        ...
        <div class="flex items-center gap-2 min-w-0">
          <Icon icon="mdi:chevron-right" class="h-4 w-4 shrink-0 text-slate-400 transition-transform"
            :class="{ 'rotate-90': !collapsedRows.has(row._rid) }" />
          <span class="text-xs font-semibold text-slate-500 uppercase tracking-wider">Row {{ rowIndex + 1 }}</span>
          <span v-if="collapsedRows.has(row._rid)" class="truncate text-xs text-slate-400">{{ repeaterRowSummary(row)
            }}</span>
        </div>
        <div class="flex items-center gap-1" @click.stop>
          <button type="button" :disabled="rowIndex === 0"
            ...
            title="Move up" @click="moveRepeaterRow(rowIndex, -1)">
            <Icon icon="mdi:chevron-up" class="w-4 h-4" />
          </button>
          <button type="button" :disabled="rowIndex === repeaterRows.length - 1"
            class="p-1 rounded text-slate-400 hover:text-slate-700 disabled:opacity-30 transition-colors"
            title="Move down" @click="moveRepeaterRow(rowIndex, 1)">
            <Icon icon="mdi:chevron-down" class="w-4 h-4" />
          </button>
          <button type="button" class="p-1 rounded text-slate-400 hover:text-red-500 transition-colors"
            title="Remove row" @click="removeRepeaterRow(rowIndex)">
            <Icon icon="mdi:close" class="w-4 h-4" />
          </button>
        </div>
      </div>
      <!-- Row fields -->
      <div v-if="!collapsedRows.has(row._rid)" class="p-3 grid gap-3" :class="subFieldGridCols">
        <div v-for="sub in (config.subfields ?? [])" :key="sub.key">
          <label class="text-xs font-medium text-slate-600 block mb-1">
            {{ sub.label || sub.key }}
            <span v-if="sub.required" class="text-red-500 ml-0.5">*</span>
          </label>
          <FieldInput :name="`${name}[${rowIndex}][${sub.key}]`" :config="sub" :model-value="row[sub.key] ?? null"
  ...
  <div v-else-if="config.type === 'color'" class="flex items-center gap-2">
    <div class="relative">
      <input type="color" :value="colorHex" :id="`${name}-color-picker`" class="sr-only"
        @input="$emit('update:modelValue', $event.target.value)" />
      <label :for="`${name}-color-picker`"
        ...
        :style="{ backgroundColor: colorHex }" :title="colorHex"></label>
    </div>
    <input type="text" :value="colorHex" class="form-input w-32 rounded-lg border-slate-300 text-sm font-mono"
      placeholder="#000000" maxlength="7" @blur="onColorHexInput($event.target.value)"
      @keydown.enter.prevent="onColorHexInput($event.target.value)" />
  </div>

  <!-- fallback -->
  <input v-else type="text" :name="name" :value="modelValue ?? ''"
    class="form-input w-full rounded-lg border-slate-300 text-sm"
    @input="$emit('update:modelValue', $event.target.value)" />
</template>
...
watch(() => props.modelValue, (val) => {
  ...
  const current = val.map((item) => String(item ?? '').trim()).filter(Boolean)
...
watch(() => props.modelValue, (val) => {
  ...
  if (props.config.type !== 'relation' && props.config.type !== 'select') return
...
const readonlyMarkdownHtml = computed(() => marked.parse(String(props.modelValue ?? ''), { async: false }))

const rangeMin = computed(() => numberOr(props.config.min, 0))
const rangeMax = computed(() => Math.max(numberOr(props.config.max, 100), rangeMin.value))
const rangeStep = computed(() => {
  const step = numberOr(props.config.step, 1)
  return step > 0 ? step : 1
})
const rangeValue = computed(() => {
  const fallback = props.config.default === null || props.config.default === undefined || props.config.default === ''
    ? rangeMin.value
    : props.config.default

  return clamp(numberOr(props.modelValue, numberOr(fallback, rangeMin.value)), rangeMin.value, rangeMax.value)
})
...
function formatRangeNumber(value) {
  ...
  const fixedDigits = decimals === null || decimals === undefined || decimals === '' ? 0 : Number(decimals)
...
async function uploadDroppedMedia(files) {
  ...
  try {
    ...
    const uploadedNames = uploaded.map((item) => uploadedMediaFilename(item)).filter(Boolean)
    ...
    emit('update:modelValue', props.config.multiple ? uploadedNames : uploadedNames.slice(0, 1))
  } catch (err) {
...
const mediaLabel = computed(() => {
  ...
  if (props.config.multiple) return `${mediaValues.value.length} media file${mediaValues.value.length === 1 ? '' : 's'} selected`
  return mediaValues.value[0]
})
const mediaPreviewTiles = computed(() => mediaValues.value.slice(0, mediaValues.value.length > 4 ? 3 : 4))
const mediaPreviewOverflow = computed(() => Math.max(mediaValues.value.length - mediaPreviewTiles.value.length, 0))
const mediaPreviewEmptyTiles = computed(() => Math.max(4 - mediaPreviewTiles.value.length - (mediaPreviewOverflow.value > 0 ? 1 : 0), 0))
...
function normalizeMediaModel(value) {
  ...
  const normalized = Array.from(new Set(source.map(extractMediaFilename).filter(Boolean)))
...
function mediaUrl(value) {
  return `/media/${encodeURIComponent(String(value))}`
}
...
const relationOptions = computed(() =>
  relationEntries.value.map((entry) => ({ value: entry.id, label: entry.title || entry.id })),
)
...
function emitRepeater() {
  emit('update:modelValue', repeaterRows.value.map(({ _rid, ...row }) => row))
}
...
function repeaterRowSummary(row) {
  for (const sub of props.config.subfields ?? []) {
    ...
    if (Array.isArray(val) && val.length > 0) return `${val.length} item${val.length === 1 ? '' : 's'}`
  }
  return '—'
}
