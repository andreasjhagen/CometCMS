<template>
  <section class="card p-5 flex flex-col gap-4">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
      <h2 class="text-sm font-semibold text-slate-900">{{ t('activity.title') }}</h2>
    ...
    <!-- List -->
    <div v-if="loading && items.length === 0" class="py-6 text-center text-sm text-slate-400">
      {{ t('activity.loading') }}
    </div>
    <div v-else-if="!loading && items.length === 0" class="py-6 text-center text-sm text-slate-400">
      {{ t('activity.empty') }}
    </div>
    <ul v-else class="divide-y divide-slate-100">
      ...
      >
        <span class="mt-1 h-2 w-2 shrink-0 rounded-full" :class="dotClass(item)" />
        <span class="min-w-0 flex-1">
          <span class="font-medium text-slate-800">{{ label(item) }}</span>
          <span v-if="sub(item)" class="ml-1.5 text-slate-500 truncate text-xs">{{ sub(item) }}</span>
        </span>
        <time
    ...
    <!-- Pagination -->
    <div v-if="meta.total > 0" class="flex items-center justify-between gap-3 pt-1">
...
function label(item) {
  return EVENT_LABELS[item.message] ? t(EVENT_LABELS[item.message]) : item.message
}
...
function dotClass(item) {
  ...
  if (msg.endsWith('.deleted') || msg.endsWith('.bulk_deleted') ||
      msg === 'content purged')                                    return 'bg-red-400'
  if (msg.endsWith('.created') || msg.endsWith('.uploaded') ||
      msg.endsWith('.restored') || msg.endsWith('.published'))     return 'bg-emerald-400'
  return 'bg-blue-400'
}
